...
try:
    ...
    from PyQt6.QtWidgets import QWizard, QWizardPage, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QSpinBox, QPushButton, QFileDialog, QComboBox, QFormLayout, QCheckBox
except ImportError:
    Qt = type("Qt", (object,), {})
    QWizard = type("QWizard", (object,), {})
...
class SourcePage(QWizardPage):
	"""First page: select installation ISO source."""
...
class ConfigPage(QWizardPage):
	"""Second page: configure VM resources."""
	...
	def initializePage(self) -> None:
		...
		if iso:
			...
			self.memory_spin.setValue(info.get("ram", 2048))
			self.disk_spin.setValue(info.get("disk", 20))


class SummaryPage(QWizardPage):
	"""Third page: review configuration before creating."""
...
class NewVMAssistant(QWizard):
	...
	def get_config(self) -> dict:
		return {
			"name": self.field("name"),
			"iso": self.field("iso"),
			"ram_mb": int(self.field("memory")),
			"cpus": int(self.field("vcpus")),
			"disk_gb": int(self.field("disk")),
		}
