...
class Type0Backend(BaseBackend):
    ...
    def _save_state(self) -> None:
        """Persist current VM states to disk."""
        data: dict[str, int] = {}
    ...
    def _load_state(self) -> dict[str, int]:
        """Load persisted VM states from disk."""
    ...
    def start_machine(self, backend_id: str) -> bool:
        ...
        # Try direct KVM first (type-0 bare-metal path)
        ...
        # Last resort: mark as running even without process
        # (allows testing without actual hypervisor)
        vm_info["state"] = MachineState.RUNNING
        self._save_state()
        return True
    ...
    def shutdown_machine(self, backend_id: str) -> bool:
        ...
        # Stop VCPU threads if any
        self._stop_vcpu_threads(backend_id)

        # Stop QEMU process if any
    ...
    # ------------------------------------------------------------------ #
    #  delete  (with disk-preservation support)
    # ------------------------------------------------------------------ #
    def delete_machine(self, backend_id: str, keep_disks: bool = False) -> bool:
        ...
        # Delete disk images unless keep_disks is set
        if not keep_disks:
    ...
    # ------------------------------------------------------------------ #
    #  Direct KVM (type-0 bare-metal path)
    # ------------------------------------------------------------------ #
    def _start_kvm_direct(
        ...
        Sets up KVM with firmware, TSS, IRQ chip, VCPUs, and launches
        VCPU threads for execution.
        """
        ...
        # Load firmware into guest memory if available
        ...
        try:
            ...
            # Create IRQ chip (PIC + IOAPIC)
            try:
                fcntl.ioctl(vm_fd, 0xAE60, 0)
            except OSError:
                pass

            # Create VCPUs and store FDs
    ...
    def _launch_vcpu_threads(
        ...
        """Launch one thread per VCPU to run KVM_RUN."""
    ...
    def _stop_vcpu_threads(self, backend_id: str) -> None:
        """Signal all VCPU threads to stop and wait for them."""
    ...
    def _run_kvm_vcpu(
        ...
        """Run a KVM VCPU loop (blocking, in background thread).

        Handles VM exits: HLT, IO, MMIO, SHUTDOWN, EINTR.
        """
        ...
        try:
            ...
            # Get VCPU fd via KVM_CREATE_VCPU
            ...
            while not stop_event.is_set():
                try:
                    ...
                    if e.errno == 4:  # EINTR
                        continue
                    if e.errno == 9:  # EBADF (VM destroyed)
                        break
                    break

                # Read exit reason from kvm_run shared memory
                try:
                    import mmap
                ...
                if exit_reason == 8:  # KVM_EXIT_HLT
                    ...
                    continue  # minimal handling
                elif exit_reason == 0x100:  # KVM_EXIT_SHUTDOWN
                    break
                elif exit_reason == 0x200:  # KVM_EXIT_FAIL_ENTRY
                    break
                elif exit_reason == 0x400:  # KVM_EXIT_INTERNAL_ERROR
                    break
                else:
                    # Unknown exit - continue (guest might still run)
                    continue
        except (OSError, ImportError):
            pass

    # ------------------------------------------------------------------ #
    #  QEMU-backed KVM path
    # ------------------------------------------------------------------ #
    def _start_qemu_kvm(
    ...
    # ------------------------------------------------------------------ #
    #  Automatic QEMU installation
    # ------------------------------------------------------------------ #
    def _auto_install_qemu(self) -> Optional[str]:
        """Download and install QEMU if missing.

        Returns the path to qemu-img or qemu-system-x86_64 on success.
        """
        ...
        # Try system package manager first (self-contained behavior)
        if shutil.which("apt"):
        ...
        # Download prebuilt QEMU static binaries
