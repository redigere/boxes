...
class Type0Backend(BaseBackend):
	...
	def shutdown_machine(self, backend_id: str) -> bool:
		...
		if proc is not None:
			...
			if qemu_pid is not None and self._podman is not None:
	...
	def _find_qemu(self) -> Optional[str]:
		path = self._podman.get_qemu_path() if self._podman is not None else None
		if path:
			return path
		for name in [
			"qemu-system-x86_64",
	...
	def _find_qemu_in_container(self, backend_id: str) -> Optional[int]:
		if not backend_id or self._podman is None:
			return None
